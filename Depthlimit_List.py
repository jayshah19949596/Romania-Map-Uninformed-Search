...
You have to give Depth number as input 

@author: jaysh
"""

# This is a Depth Limited Search
# The problem of the program is stored in a dictionary
# The program is always complete if the goal is present
# Variable path represents the path of the Depth Limited Search
# Variable s represent the current state of the Depth Limited Search
...
def graph_search(start,goal,problem,depth_limit):
    ...
    search_finished  = False # Represents whether the search has finished or not with in the given depth limit
    print ('\n Initialisation value of frontier = '+", ".join(frontier))

    
    while frontier and i < int(depth_limit):
        print("This is limit number "+str(i+1))
        i = i + 1
    ...
    if(search_finished == True):
        ...
        actual_path,cost = backtracking(start,goal,parent,path)
        ...
        print ("\n Final Path Cost from Arad To "+path[-1]+" = "+str(total_path_cost))        
...
def backtracking(start,goal,parent,path):
    reverse_path = [path[-1]]
...
start = "Arad"     
goal = "Bucharest"
depth_limit = input("What is the limit of the depth that u want to set \n ")  #Takes the input of the till what depth limit the search should go
graph_search(start,goal,problem,depth_limit)
...
total_path_cost = calculate_path_cost(path,problem)
    ...
    if(search_finished == True):
        ...
        print ("\n Final Path Cost from Arad To Bucharest = "+str(total_path_cost))
